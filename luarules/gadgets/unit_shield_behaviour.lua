...
local function getTeamShieldPowerMultiplier(unitID)
	if GG.TeamOptions and GG.TeamOptions.GetEffectiveNumericByTeamID then
		local teamID = Spring.GetUnitTeam(unitID)
		if teamID then
			local mult = GG.TeamOptions.GetEffectiveNumericByTeamID(teamID, "multiplier_shieldpower")
...
function gadget:ShieldPreDamaged(
	...
	if proID > -1 then
		...
		newShieldDamage = newShieldDamage / getTeamShieldPowerMultiplier(shieldUnitID)
	...
	elseif beamEmitterUnitID then
		...
		local beamShieldDamage = originalShieldDamages[weaponDefID] or 0
		beamShieldDamage = beamShieldDamage / getTeamShieldPowerMultiplier(shieldUnitID)
		shieldData.shieldDamage = shieldData.shieldDamage + beamShieldDamage
	end
...
local function addCustomShieldDamage(shieldUnitID, damage, weaponDefID)
	...
	if state == SHIELDSTATE_ENABLED and power > 0 then
		...
		damage = damage / getTeamShieldPowerMultiplier(shieldUnitID)
