...
local function SpawnUnit(spawnData)
	...
	if spawnDef then
		...
		if spawnDef.debris then
			local debrisName = spawnDef.debris
			if GG.TeamTweaks and GG.TeamTweaks.ResolveUnitName then
				debrisName = GG.TeamTweaks.ResolveUnitName(spawnData.teamID, debrisName)
			end
			local debrisID = spCreateUnit(debrisName, x, spawnData.y, z, 0, spawnData.teamID)
			if debrisID then
		...
		if spawnDef.feature then
			...
			if GG.TeamTweaks and GG.TeamTweaks.ResolveUnitName and spawnUnitName then
				spawnUnitName = GG.TeamTweaks.ResolveUnitName(spawnData.teamID, spawnUnitName)
			end
