...
local function getTeamConversionMultiplier(unitID)
	if GG.TeamOptions and GG.TeamOptions.GetEffectiveNumericByTeamID then
		...
		if teamID then
			return
				...
				* GG.TeamOptions.GetEffectiveNumericByTeamID(teamID, "multiplier_energyconversion")
		end
	end
	return 1
end
...
local function updateUnitConversion(unitID, unitData, amount, e)
	...
	spSetUnitResourcing(unitID, "umm", amount * e * getTeamConversionMultiplier(unitID))
