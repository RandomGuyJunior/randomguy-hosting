...
local wereZombies = {}
local pendingUnitXp = {}
local pendingCorpseModes = {}
local zombieModesByUnit = {}
...
local getRezPowerSpeedForTechLevel

local function calculateSpawnDelayFramesForConfig(unitPower, config)
	local techLevel = currentTechLevel or 1
	local rezPowerSpeed = getRezPowerSpeedForTechLevel(config, techLevel)
	local spawnSeconds = floor(unitPower / rezPowerSpeed)
	spawnSeconds = clamp(spawnSeconds, config.rezMin, config.rezMax)
	return spawnSeconds * Game.gameSpeed
end

local function calculateSpawnDelayFrames(unitPower)
	return calculateSpawnDelayFramesForConfig(unitPower, currentZombieConfig)
end

getRezPowerSpeedForTechLevel = function(config, techLevel)
...
local function rollSpawnCount(config)
	return random(config.countMin, config.countMax)
end

local function calculateSpawnCount(unitDefID, config)
	local countMin = config.countMin
	local countMax = config.countMax
	if countMin == countMax then
		return countMin
	end

	local unitDef = unitDefs[unitDefID]
	local rezTimeSeconds = calculateSpawnDelayFramesForConfig(getUnitRezPower(unitDef), config) / Game.gameSpeed
	local rezMin = config.rezMin
	local rezMax = config.rezMax

	if currentTechLevel <= 1 then
		return math.min(rollSpawnCount(config), rollSpawnCount(config), rollSpawnCount(config)) -- extra min() rolls skew the count down except for cheap, fast-rez units
	end

	if rezTimeSeconds == rezMin then
		return rollSpawnCount(config)
	end
	if rezTimeSeconds == rezMax then
		return math.min(rollSpawnCount(config), rollSpawnCount(config), rollSpawnCount(config))
	end
	return math.min(rollSpawnCount(config), rollSpawnCount(config))
end
...
local function spawnZombies(featureID, unitDefID, healthReductionRatio, x, y, z, wasZombie, pastXp, zombieMode)
	...
	if not wasZombie and unitDef.speed > 0 then
		spawnCount = calculateSpawnCount(unitDefID, config)
	end
...
local function queueCorpseForSpawning(featureID, override, wasZombie, pastXp, zombieMode)
	if not override and not autoSpawningEnabled then
		return
	end

	if not zombieMode then
		zombieMode = modOptionEnabled and (modOptions.zombies or "normal") or nil
	end
	if not override and (not zombieMode or not zombieModeConfigs[zombieMode]) then
		return
	end
	if override and (not zombieMode or not zombieModeConfigs[zombieMode]) then
		zombieMode = currentZombieMode
	end
	...
	local config = zombieModeConfigs[zombieMode] or currentZombieConfig
	local spawnDelayFrames
	if corpseDefData.neverRespawn then
		return
	elseif corpseDefData.customRespawnTime then
		spawnDelayFrames = floor(corpseDefData.customRespawnTime * Game.gameSpeed)
	else
		local unitDef = unitDefs[corpseDefData.unitDefID]
		spawnDelayFrames = calculateSpawnDelayFramesForConfig(getUnitRezPower(unitDef), config)
	end
...
local function getZombieModeForTeam(teamID)
	if modOptionEnabled then
		return modOptions.zombies or "normal"
	end
	...
	local mode = teamOptions.GetZombieMode(slot, "disabled")
	return mode ~= "disabled" and mode or nil
end
...
function gadget:UnitDestroyed(unitID, unitDefID, unitTeam)
	...
	local unitZombieConfig = zombieMode and zombieModeConfigs[zombieMode]
	if isZombie(unitID) and unitZombieConfig and unitZombieConfig.zombieCorpses and not heapingZombies[unitID] then
