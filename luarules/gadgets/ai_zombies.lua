...
local function isZombieFriendlyTeam(teamID)
	if teamID == nil then
		return false
	end
	local _, _, _, _, _, allyTeamID = spring.GetTeamInfo(teamID)
	return allyTeamID ~= nil and zombieFriendlyAllyTeams[allyTeamID] == true
end
...
local function getCombatTargetData(unitDefID, targetID)
	...
	if
		not targetTeamID
		or spAreTeamsAllied(gaiaTeamID, targetTeamID)
		or isZombieFriendlyTeam(targetTeamID)
	then
		return
	end
...
local function assignZombieAggroEvenly()
	...
	for allyTeamID, allyPower in pairs(allyPowers) do
		if
			not zombieFriendlyAllyTeams[allyTeamID]
			and allyPower > 0
			and getRandomValidAllyTeamUnit(allyTeamID)
		then
			local allyData = {
...
local function aggroTeamID(teamID)
	local _, _, isDead, _, _, allyTeamID = spring.GetTeamInfo(teamID)
	if isDead ~= false or not allyTeamID or zombieFriendlyAllyTeams[allyTeamID] then
		return false
	end
...
local function aggroAllyID(allyTeamID)
	...
	if not allyTeams or #allyTeams == 0 or zombieFriendlyAllyTeams[allyTeamID] then
		return false
	end
	return aggroAllZombiesToAllyTeam(allyTeamID)
end
...
function gadget:Initialize()
	...
	GG.ZombieAI = {
		...
		ZombieFriendlyAllyTeams = zombieFriendlyAllyTeams,
	}
end
...
function gadget:AllowWeaponTarget(attackerID, targetID)
	if
		isZombie(attackerID)
		and targetID
		and isZombieFriendlyTeam(spGetUnitTeam(targetID))
	then
		return false
	end
	return true
